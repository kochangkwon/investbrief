...
_MATERIALITY_RE = re.compile(r"MATERIALITY:\s*(HIGH|MEDIUM|LOW)", re.IGNORECASE)
...
async def verify_theme_with_claude(
    ...
    *,
    ...
    """VERDICT + MATERIALITY + REASON 파싱 (지시서 F).

    Returns (verdict, materiality, reason):
        - verdict: True/False/None(실패)
    ...
    verdict = verdict_match.group(1).upper() == "YES"
    mat_match = _MATERIALITY_RE.search(raw)
    materiality = mat_match.group(1).upper() if mat_match else None
    reason_match = _REASON_RE.search(raw)
    reason = reason_match.group(1).strip() if reason_match else "(근거 미파싱)"
    return verdict, materiality, reason
