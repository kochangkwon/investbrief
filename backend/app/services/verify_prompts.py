...
다음 **네 조건을 모두** 만족할 때만 YES:

조건 0 — 종목 지칭 여부 (**가장 먼저 확인**):
- 뉴스에서 "{stock_name}"이 **회사(상장사)를 가리키는 고유명사**로 쓰였는지 확인하세요
- 같은 글자가 **일반명사·부사로 쓰인 경우 무조건 NO**입니다
  (예: "환자를 **대상**으로"·"상장사 **대상**" / "**전방** 산업" / "해커 **신원**" /
   "2**나노** 공정" / "자금 **동원**" / "**한창** 진행 중")
- 종목명이 기사 **제목에 없고 본문에만** 등장한다면 특히 의심하세요 — 회사 언급이
  아니라 일반명사일 가능성이 큽니다
- 회사로 인정하는 근거: 회사 접미("그룹"·"홀딩스"·"㈜"), 기사 주어 위치, 실적·공시·
  사업 맥락에서의 언급
- **이 조건이 아니면 아래 조건은 볼 필요 없이 NO입니다**

조건 1 — 실질 관련성:
- 종목의 **주력 사업(매출 비중이 큰 핵심 사업)**이 이 테마와 직접 관련 있어야 함
- 그룹 지주회사가 계열사 이슈로 언급된 경우는 NO (예: 방산 뉴스의 "한화"는 한화에어로스페이스가 수혜주이지 지주사 한화가 아님)
- **헤드라인 축약 주의 (실사고 2회)**: 기사 제목은 계열사명을 그룹명으로 줄여 쓰는
  관행이 있습니다 — "효성, 美빅테크 수주"(실제 주체 효성중공업), "HD현대·효성
  한발 앞서"(효성중공업). 제목이 "{stock_name}"을 행위 주체로 표기해도, 사업
  내용(수주 품목·공장·기술: 변압기·HVDC·방산·조선 등)이 **특정 계열사의 주력**이면
  그룹 동명의 지주사·다른 상장사는 NO입니다. "{stock_name}"이 지주회사라면
  계열사의 수주·실적·투자 뉴스는 전부 NO — 지주사는 그 재료의 직접 수혜
  위치가 아닙니다 (이 오판이 과거 −25% 실손실을 만들었습니다)
...
**애매하면 NO.** 확신이 없으면 NO.
**특히 조건 0(종목 지칭 여부)이 애매하면 반드시 NO** — 일반명사 오인식이 실제
매수 주문 직전까지 간 사고가 있었습니다.
